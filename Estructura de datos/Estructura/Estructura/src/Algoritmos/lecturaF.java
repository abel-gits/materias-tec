...
public class lecturaF {
	...
	public static void buscar(Titulos datos[]){
		archivo(datos);
		quicksort(datos,0,32953);
		System.out.println("escribe el titulo a buscar");
		String dato=Leer.dato();
		int n = datos.length;
		int centro=0,inf=0,sup=n-1;
		int L=dato.length();
		String Leido=datos[centro].getTitulo().substring(0,L);
		while(inf<=sup){
			centro=(sup+inf)/2;
			Leido=datos[centro].getTitulo().substring(0,L);
			//System.out.println("Buscado-> "+dato+" L="+L+" Leido-> "+Leido+" L="+Leido.length());
			System.out.println(dato+"   "+Leido);
			if(dato.equals(Leido)) 
			{
				System.out.println("TITULO ENCONTRADO: "+datos[centro].getTitulo());
				
			}
			else 
				if(dato.compareTo(Leido) < 0)
				{
					sup=centro-1;
				}
				else 
